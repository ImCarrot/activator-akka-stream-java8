...
public class TcpEcho {
    ...
    public static void server(final ActorSystem system, final InetSocketAddress serverAddress) {
        ...
        final CompletionStage<ServerBinding> bindingFuture = Tcp.get(system).bind(serverAddress.getHostString(), serverAddress.getPort()).to(handler).run(materializer);

        bindingFuture.handle((final ServerBinding binding, final Throwable exception) -> {
            if (binding != null) {
    ...
    public static void client(final ActorSystem system, final InetSocketAddress serverAddress) {
        final ActorMaterializer materializer = ActorMaterializer.create(system);
        ...
        final Source<ByteString, NotUsed> responseStream = Source.from(testInput).via(Tcp.get(system).outgoingConnection(serverAddress.getHostString(), serverAddress.getPort()));

        final CompletionStage<ByteString> result = responseStream.runFold(
